...
function StepOption({ label, description, icon, isActive, onClick }: StepOptionProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      {icon && (
        <span className="mt-0.5 text-lg sm:text-xl shrink-0">
          {icon}
        </span>
      )}
      <span>
...
function barOptionsSurcharge(options: BarOption[]): number {
  // Надбавка за наполнение бара, максимум 30%
  let percent = 0;
...
export default function BarCalculatorQuiz() {
  ...
  // Метрика: открытие блока
  useEffect(() => {
    reachGoalAll("calc_open");
  }, []);

  // Метрика: использование и пересчёт
  useEffect(() => {
  ...
  const { approxPrice, surchargePercent, guestsCount } = useMemo(() => {
    ...
    const perGuest = 100; // минимальная ставка из формулы 100–150 ₽
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payloadLines = [
        ...
        `Примерная стоимость: от ${formatPrice(
          approxPrice
        )} ₽ / день`,
        "",
      ...
      // Даже если отправка не удалась (например, нет Telegram‑токена на локальной машине),
      // показываем клиенту рассчитанную стоимость, а менеджеру выводим текст ошибки.
      setIsSubmitted(true);
      setCurrentStep(5);
      setErrorMessage(
        error?.message ||
      ...
      <div className="rounded-3xl border border-amber-400/40 bg-black/40 backdrop-blur-sm shadow-2xl shadow-amber-500/15 overflow-hidden">
        <div className="border-b border-amber-400/20 bg-gradient-to-r from-amber-500/20 via-yellow-400/10 to-transparent px-4 sm:px-6 py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm text-amber-100/80">
            ...
            <span>Москва и МО • Работаем по договору • Документы для юрлиц</span>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-[minmax(0,3fr)_minmax(0,2.2fr)] gap-6 lg:gap-8">
            ...
            {currentStep === 4 && (
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-amber-300/40 bg-black/30 p-4 sm:p-5 space-y-3 text-amber-100/90 text-sm sm:text-base">
                  <p>
                    Мы считали примерную стоимость по формуле:
                  </p>
                  <p className="font-mono text-xs sm:text-sm bg-black/40 rounded-lg px-3 py-2">
                    25 000 ₽ + (гости × 100–150 ₽) + надбавка за
                    наполнение 0–30%
                  </p>
                  <p className="text-amber-100/80">
                    Чтобы увидеть результат, оставьте контакты — менеджер
                    уточнит детали и пришлёт точную смету и варианты меню
                    в течение 15 минут.
                  </p>
                  <p className="text-xs sm:text-sm text-amber-200/70">
                    После отправки формы мы свяжемся с вами удобным
                    способом и подскажем, как оптимизировать бюджет под
                    ваш формат события.
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-4 lg:space-y-5">
              ...
              <p className="text-[11px] sm:text-xs text-amber-200/60">
                Стоимость пересчитывается в реальном времени по вашим
                ответам и носит ориентировочный характер.
              </p>
            </div>
              ...
              {isSubmitted ? (
                <div className="space-y-2">
                  ...
                  <p className="text-sm sm:text-base text-amber-100/80">
                    Мы свяжемся с вами в течение 15 минут, уточним детали
                    и пришлём точную смету и варианты меню.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <p className="text-xs sm:text-sm text-amber-100/80">
                    Оставьте контакты — мы пришлём точную смету и
                    варианты меню в течение 15 минут.
                  </p>
                  ...
                  <p className="mt-2 text-[11px] sm:text-xs text-amber-200/60 text-center">
                    Москва и МО • Работаем по договору • Документы для
                    юрлиц
                  </p>
                </form>
              )}
